...
from .serializers import (
    ...
    ProductDetailSerializer, MyTokenObtainPairSerializer, CategoryListSerializer, ProfileSerializer
)
...
class CartItemCreateView(APIView):
    ...
    def post(self, request, format=None):
        ...
        cart, created = Cart.objects.get_or_create(
            profile=request.user.profile, completed=False)
        ...
        if quantity == 0:
            ...
            if not cart_item.exists():
                cart_item = CartItem.objects.create(item=Product.objects.get(
                    id=product_id), cart=cart, quantity=int(validated_data['quantity']))
            else:
    ...
    def get(self, request, format=None):
        try:
            cart = Cart.objects.get(
                completed=False, profile=request.user.profile)
        except:
